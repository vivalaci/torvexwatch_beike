<?php

/**
 * design_builder.php
 *
 * @copyright  2022 beikeshop.com - All Rights Reserved
 * @link       https://beikeshop.com
 * @author     guangda <service@guangda.work>
 * @created    2022-08-30 11:03:19
 * @modified   2022-08-30 11:03:19
 */

return [
    'module_brand'              => 'Brand',
    'module_banner'             => 'Banner',
    'module_four_image_pro'     => 'For Image PRO',
    'module_image_402'          => 'Four image Pro-2',
    'module_image_403'          => 'Four image - 3',
    'module_slideshow'          => 'Slideshow',
    'module_tab_products'       => 'Tab Products',
    'module_product'            => 'Products',
    'module_icons'              => 'Icons',
    'module_rich_text'          => 'Rich Text',
    'module_page'               => 'Article module',
    'design_index'              => 'Home Decoration',
    'design_header_index'       => 'Header decoration',
    'design_footer_index'       => 'Last page decoration',
    'design_app_home_index'     => 'APP homepage design',
    'module_img_text_slideshow' => 'Graphic slides',
    'module_img_text_slideshow_2' => 'Graphic slides 2',
    'module_img_text_banner'    => 'Graphic banner',
    'app_push_index'            => 'APP message push',
    'theme_index'               => 'Template list',
    'theme_update'              => 'Template update',
    'design_menu_index'         => 'Navigation list',
    'design_menu_update'        => 'Navigation update',
    'design_update'             => 'Home page decoration update',
    'design_header_update'      => 'Header decoration update',
    'design_footer_update'      => 'Final page decoration update',
    'design_app_home_update'    => 'APP home page design update',
    'module_img_text_banner_multiple'              => 'Multi-image text banner',
];
