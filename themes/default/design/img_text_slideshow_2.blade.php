...
<section class="module-item {{ $design ? 'module-item-design' : ''}}" id="module-{{ $module_id }}">
          ...
          @foreach($content['images'] as $image)
            <div class="swiper-slide">
              <div class="image-wrap">
                ...
                <video src="{{ $image['image'] }}" class="img-fluid module-img-text-slideshow-2__video" muted loop playsinline autoplay preload="metadata"></video>
                @else
  ...
  @php
    $luxSwiperHero2Count = count($content['images'] ?? []);
    $luxSwiperHero2Loop = $luxSwiperHero2Count >= 2;
  @endphp
  <script>
    var moduleSwiperImgText_{{ $module_id }} = new Swiper ('.module-swiper-img-text-{{ $module_id }}', {
      loop: @json($luxSwiperHero2Loop),
      parallax : true,
