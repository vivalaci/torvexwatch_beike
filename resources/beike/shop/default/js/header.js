...
$(function () {
  ...
  // 首页：整颗 header 叠在首屏上，滚动后 fixed + 顶栏黑 / 主导航白（lux-header-*）
  (function () {
    ...
    function updateHomeHeader() {
      ...
      if (scrollTop > threshold) {
        $body.addClass('lux-header-scrolled');
        $header.removeClass('lux-header-over-hero').addClass('lux-header-fixed');
        if (!$placeholder.length) {
          $placeholder = $('<div class="lux-header-placeholder"></div>').insertBefore($header);
        }
        $placeholder.css({ height: headerH, display: 'block' });
      } else {
        $body.removeClass('lux-header-scrolled');
        $header.removeClass('lux-header-fixed').addClass('lux-header-over-hero');
        if ($placeholder.length) {
    ...
    $header.addClass('lux-header-over-hero');
    updateHomeHeader();
    $(window).on('scroll.luxHomeHeader resize.luxHomeHeader', updateHomeHeader);
  }());

  // PC/MB 头部滑动固定（非首页沿用原逻辑）
  (function () {
    if ($('body').hasClass('page-home')) return;
